  ...
  return (
    <div style={{ position: 'relative' }}>
      ...
      >
        ...
        <div style={{ width: '36px', height: '36px', borderRadius: '50%', background: 'var(--navy-m)', border: '0.5px solid var(--bd1)', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: '5px', overflow: 'hidden', flexShrink: 0 }}>
          {slot.politician.portrait_url
            ...
            : <svg width="22" height="28" viewBox="0 0 22 28" fill="none" style={{ opacity: 0.35 }}><ellipse cx="11" cy="9" rx="6" ry="7" fill="#ece8dc"/><path d="M2 28C2 19 5 16 11 15C17 16 20 19 20 28" fill="#ece8dc"/></svg>
          }
        </div>
        {(() => {
          // Show first name + surname truncated — more readable than surname-only
          const parts = slot.politician.name.split(' ')
          const displayName = parts.length >= 2
            ? `${parts[0]} ${parts[parts.length - 1]}`
            : slot.politician.name
          return (
            <div style={{ fontSize: '9px', fontWeight: 500, color: isOpp ? 'var(--t3)' : 'var(--t1)', lineHeight: 1.2, marginBottom: '2px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', width: '100%' }}>
              {displayName}
              {isOpp && (
