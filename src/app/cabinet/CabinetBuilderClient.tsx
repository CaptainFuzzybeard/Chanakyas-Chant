...
export function CabinetBuilderClient({
  ...
  return (
    <>
      ...
      <div className="layout-cabinet" style={{ display: 'grid', gridTemplateColumns: '1fr 300px', minHeight: 'calc(100vh - 88px)' }}>
                ...
                {slot ? (
                  <div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                      <div style={{ width: '34px', height: '34px', borderRadius: '50%', background: 'var(--navy-m)', border: '0.5px solid var(--bd1)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, overflow: 'hidden' }}>
                        {slot.politician.portrait_url
                          // eslint-disable-next-line @next/next/no-img-element
                          ? <img src={slot.politician.portrait_url} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                          : <svg width="20" height="26" viewBox="0 0 20 26" fill="none" style={{ opacity: 0.35 }}><ellipse cx="10" cy="8" rx="5" ry="6" fill="#ece8dc"/><path d="M2 26C2 18 5 15 10 14C15 15 18 18 18 26" fill="#ece8dc"/></svg>
                        }
                      </div>
      ...
      <div style={{ padding: '18px 16px', background: 'var(--bg1)', overflowY: 'auto' }}>
        ...
        {/* Party diversity bar — shows party cap usage */}
        {partyBreakdown.length > 0 && (
          <div style={{ marginBottom: '14px' }}>
            <div style={{ fontSize: '8.5px', fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', color: 'var(--t3)', marginBottom: '7px', display: 'flex', alignItems: 'center', gap: '6px' }}>
              Coalition mix
              <span style={{ flex: 1, height: '0.5px', background: 'var(--bd0)' }} />
              <span style={{ fontFamily: '"DM Mono",monospace', fontSize: '9px', color: 'var(--t4)', fontWeight: 400, textTransform: 'none', letterSpacing: 0 }}>max 4 per party</span>
            </div>
            <div style={{ display: 'flex', gap: '1px', height: '6px', borderRadius: '3px', overflow: 'hidden' }}>
              {partyBreakdown.map((p: PartyBreakdown) => (
                <div
                  key={p.party_id}
                  title={`${p.abbreviation}: ${p.count}/4`}
                  style={{
                    flex: p.count,
                    background: p.count >= 4 ? 'var(--red)' : p.count >= 3 ? 'var(--amber)' : 'var(--green)',
              ...
              {slots.length < 20 && (
                <div style={{ flex: 20 - slots.length, background: 'var(--bd0)', minWidth: '2px' }} />
              )}
            </div>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '3px', marginTop: '6px' }}>
              {partyBreakdown.slice(0, 6).map((p: PartyBreakdown) => (
                <span
                  key={p.party_id}
                  style={{
                    fontSize: '8.5px',
                    ...
                    padding: '1px 5px',
                    borderRadius: '2px',
                    background: p.count >= 4 ? 'rgba(192,57,43,.1)' : 'var(--bg3)',
                    color: p.count >= 4 ? 'var(--red)' : 'var(--t3)',
                    border: p.count >= 4 ? '0.5px solid rgba(192,57,43,.25)' : 'none',
                  }}
                >
                  {p.abbreviation} {p.count}/4
                </span>
              ))}
            </div>
          </div>
              ...
              >
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontSize: '11.5px', fontWeight: 500, color: isPending ? 'var(--red)' : 'var(--t1)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                    ...
                    {isOpp && <span style={{ fontSize: '9px', color: 'rgba(120,170,224,0.85)', marginLeft: '5px' }}>opp</span>}
                  </div>
