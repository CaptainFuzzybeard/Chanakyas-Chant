    ...
    <>
      ...
      <div>
        ...
        <div style={{ background: 'var(--bg1)', border: '0.5px solid var(--bd1)', borderRadius: '14px', overflow: 'hidden', marginBottom: '16px' }}>
          {/* Portrait */}
          <div style={{ height: '220px', background: 'var(--bg2)', position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden' }}>
            {pol.portrait_url ? (
            ...
            >
              {(score, delta7d, justUpdated) => (
                <div style={{ position: 'absolute', top: '14px', right: '14px', background: 'rgba(12,12,10,.82)', border: `0.5px solid ${justUpdated ? 'var(--amber)' : 'var(--bd1)'}`, borderRadius: '12px', padding: '8px 12px', textAlign: 'center', backdropFilter: 'blur(6px)', zIndex: 2, transition: 'border-color 0.3s' }}>
                  <ScoreExplainer stats={stats} politicianName={pol.name}>
                    {/* ScoreExplainer wraps the score — tap to explain */}
                  </ScoreExplainer>
                  <InfoPop {...INFO.composite_score}>
                    <div style={{ fontFamily: '"DM Mono",monospace', fontSize: '22px', fontWeight: 500, color: justUpdated ? 'var(--amber-b)' : 'var(--t1)', lineHeight: 1, transition: 'color 0.3s' }}>{score}</div>
                  </InfoPop>
                  {delta7d != null && (
        ...
        <div style={{ background: 'var(--bg1)', border: '0.5px solid var(--bd0)', borderRadius: '14px', padding: '16px 18px', marginBottom: '16px' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '14px' }}>
            ...
            <div style={{ fontSize: '10px', color: 'var(--t3)' }}>
              Score breakdown: <span style={{ cursor: 'pointer', color: 'var(--amber)', textDecoration: 'underline dotted' }}
                onClick={() => {/* ScoreExplainer triggered on score number */}}
              >tap the score above</span>
            </div>
          </div>
